defmodule PillPalWeb.CoreComponents do
  @moduledoc """
  ...
  attr :progress, :integer,
    default: 0,
    doc: "the current progress of the bar.Should be an integer from 0 to 100"

  def circular_progress_bar(assigns) do
    radius = assigns.radius

    # Calculate the circumference which is equal to the 100 progress.
    circumference = 2 * 3.142 * radius

    # Convert the current progress to offset
    assigns =
      assigns
      |> assign(:cx, radius + 10)
      |> assign(:cy, radius + 10)
      |> assign(:width, radius * 2 + 20)
      |> assign(:height, radius * 2 + 20)
      |> assign(:circumference, circumference)
      |> assign(
        :offset,
        circumference - assigns.progress / 100 * circumference
      )
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(PillPalWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(PillPalWeb.Gettext, "errors", msg, opts)
    end
  end
